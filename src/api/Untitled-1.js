...
for (let i = 0; i < USERS.length; i++) {
  ...
  REQUEST.param("user", USER_ID).param("foo", "bar");

  // set custom header(s)
  REQUEST.header("X-MyApp-Session", SESSION_ID).header(
    "X-MyApp-Time",
    CURRENT_UTC_TIME.format("YYYY-MM-DD HH:mm:ss"),
  );

  // set custom body from a file, e.g.
  REQUEST.body(await $fs.readFile(`/path/to/user/data/user_${USER_ID}.json`));
